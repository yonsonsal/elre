...
public class PosicionesRecorridoCalcs {
	...
	public static String getDireccion(String gid, Usuario usuario) throws Exception {
		String geometria = Datahandler.selectOneValue(datasourceOracle,
				"select SDO_UTIL.TO_WKTGEOMETRY(THE_GEOM) from df_posiciones_recorrido where gid=" + gid, usuario);
	...
	public static double getX(String gid, Usuario usuario) throws Exception{
		String geometria = Datahandler.selectOneValue(datasourceOracle,
				"select SDO_UTIL.TO_WKTGEOMETRY(THE_GEOM) from df_posiciones_recorrido where gid=" + gid, usuario);
				JSONArray colXeY = ServiceDFR.getXeY(geometria);
		JSONObject datosXeY = (JSONObject) colXeY.get(0);
	...
	public static double getY(String gid, Usuario usuario) throws Exception{
		String geometria = Datahandler.selectOneValue(datasourceOracle,
				"select SDO_UTIL.TO_WKTGEOMETRY(THE_GEOM) from df_posiciones_recorrido where gid=" + gid, usuario);
				JSONArray colXeY = ServiceDFR.getXeY(geometria);
		JSONObject datosXeY = (JSONObject) colXeY.get(0);
	...
	public static String getMotivoInactivo(String gid, Usuario usuario) throws Exception{
		String motivo = Datahandler.selectOneValue(datasourceOracle,
				"select motivo from v_df_posiciones_recorrido_geom where gid=" + gid, usuario);
		return motivo;
	}
