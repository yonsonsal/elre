const cantidadCaracteresSeparacion = 50;
...
(function (global, factory) {
  ...
  /**
	 ...
	 * @name Filter
	 * @description [filter
	 *              operators](http://schemas.opengis.net/filter/1.1.0/filter.xsd),
	 *              see also
	 *              [geoserver](http://docs.geoserver.org/stable/en/user/styling/sld/reference/filters.html)
	 * @property {string} type Can be 'comparison', 'and', 'or', 'not', or
	 *           'featureid'.
	 * @property {Array<string>} [fids] An array of feature id's. Required for
	 *           type='featureid'.
	 * @property {string} [operator] Required for type='comparison'. Can be one
	 *           of 'propertyisequalto', 'propertyisnotequalto',
	 ...
	 *           An array of filter predicates that must all evaluate to true
	 *           for 'and', or for which at least one must evaluate to true for
	 *           'or'.
	 * @property {Filter} [predicate] Required for type='not'. A single
	 *           predicate to negate.
	 ...
	 * @property {string} [lowerboundary] Lower boundary, required for
	 *           operator='propertyisbetween'.
	 * @property {string} [upperboundary] Upper boundary, required for
	 *           operator='propertyisbetween'.
	 * @property {string} [wildcard] Required wildcard character for
	 ...
	 * @property {string} [escapechar] Required escape character for
	 *           operator='propertyislike'.
	 */
  ...
  /**
	 * Generic parser for elements with maxOccurs > 1 it pushes result of
	 * readNode(node) to array on obj[prop]
	 * 
	 * @private
	 * @param {Element}
  ...
  /**
	 * Generic parser for maxOccurs = 1 (the xsd default) it sets result of
	 * readNode(node) to array on obj[prop]
	 * 
	 * @private
	 * @param {Element}
	 ...
	 *            obj the object to modify
	 * @param {string}
	 *            prop key on obj to hold empty object
	 */
  ...
  /**
	 ...
	 *            prop [description]
	 * @param {bool}
	 *            [trimText] Trim whitespace from text content (default false).
	 */
  ...
  /**
	 * This function parses SLD XML nodes that can contain an SLD filter
	 * expression. If the SLD node contains only text elements, the result will
	 * be concatenated into a string. If the SLD node contains one or more
	 * non-literal nodes (for now, only PropertyName), the result will be an
	 * object with type:"expression" and an array of child nodes of which one or
	 * more have the type "propertyname".
	 * 
	 * Functions and arithmetic operators (Add,Sub,Mul,Div) are not supported
	 * (yet). Note: for now, only these contents will be parsed: * Plain text
	 * nodes. * CDATA sections. * ogc:PropertyName elements (property name will
	 * be parsed as trimmed text). * ogc:Literal elements (contents will be
	 * parsed as trimmed text). See also: *
	 ...
	 *            node XML Node.
	 * @param {object}
	 *            obj Object to add XML node contents to.
	 * @param {string}
	 *            prop Property name on obj that will hold the parsed node
	 *            contents.
	 * @param {bool}
	 *            [skipEmptyNodes] Default true. If true, emtpy
	 *            (whitespace-only) text nodes will me omitted in the result.
	 */
  ...
  /**
	 * Each propname is a tag in the sld that should be converted to plain
	 * object
	 * 
	 * @private
	 * @type {Object}
  ...
  /**
	 ...
	 * @private
	 * @param {Element}
	 *            node derived from xml
	 * @param {object}
	 *            obj recieves results
	 * @return {void}
	 */
  ...
  /**
	 ...
	 *              {@link http://schemas.opengis.net/sld/1.1/StyledLayerDescriptor.xsd xsd}
  ...
  /**
	 * @typedef Layer
	 * @name Layer
	 * @description a typedef for Layer, the actual style object for a single
	 *              layer
	 * @property {string} name layer name
	 * @property {Object[]} styles See explanation at [Geoserver
	 *           docs](http://docs.geoserver.org/stable/en/user/styling/sld/reference/styles.html)
	 * @property {Boolean} styles[].default
	 * @property {String} [styles[].name]
	 * @property {FeatureTypeStyle[]} styles[].featuretypestyles Geoserver will
	 *           draw multiple, libraries as openlayers can only use one
	 *           definition!
	 */
  ...
  /**
	 ...
	 *              {@link http://schemas.opengis.net/se/1.1.0/FeatureStyle.xsd xsd}
	 * @property {Rule[]} rules
	 */
  ...
  /**
	 ...
	 * @description a typedef for
	 *              [PolygonSymbolizer](http://schemas.opengis.net/se/1.1.0/Symbolizer.xsd),
	 *              see also [geoserver
	 *              docs](http://docs.geoserver.org/stable/en/user/styling/sld/reference/polygonsymbolizer.html)
	 * @property {Object} fill
	 * @property {array} fill.css one object per CssParameter with props name
	 *           (camelcased) & value
  ...
  /**
	 ...
	 * @name LineSymbolizer
	 * @description a typedef for
	 *              [LineSymbolizer](http://schemas.opengis.net/se/1.1.0/Symbolizer.xsd),
	 *              see also [geoserver
	 *              docs](http://docs.geoserver.org/stable/en/user/styling/sld/reference/linesymbolizer.html#sld-reference-linesymbolizer)
	 * @property {Object} stroke
	 * @property {Object[]} stroke.css one object per CssParameter with props
	 *           name (camelcased) & value
  ...
  /**
	 ...
	 * @description a typedef for PointSymbolizer
	 *              [xsd](http://schemas.opengis.net/se/1.1.0/Symbolizer.xsd) &
	 *              [geoserver
	 *              docs](http://docs.geoserver.org/latest/en/user/styling/sld/reference/pointsymbolizer.html)
  ...
  /**
	 * A very basic implementation of a PropertyIsLike by converting match
	 * pattern to a regex.
	 * 
	 * @private
	 * @param {object}
	 *            comparison filter object for operator 'propertyislike'
	 * @param {string|number}
	 *            value Feature property value.
	 * @param {object}
	 *            getProperty A function with parameters (feature, propertyName)
	 *            to extract the value of a property from a feature.
	 */
  ...
  /**
	 ...
	 * @private
	 * @param {Filter}
	 *            comparison A comparison filter object.
	 * @param {object}
  ...
  /**
	 * @private Get feature properties from a GeoJSON feature.
	 * @param {object}
  ...
  /**
	 * @private Gets feature id from a GeoJSON feature.
	 * @param {object}
	 *            feature GeoJSON feature.
  ...
  /**
	 ...
	 *            feature feature
	 * @param {object}
	 *            options Custom filter options.
	 * @param {Function}
	 *            options.getProperty An optional function with parameters
	 *            (feature, propertyName) that can be used to extract properties
	 *            from a feature. When not given, properties are read from
	 *            feature.properties directly.
	 * @param {Function}
	 *            options.getFeatureId An optional function to extract the
	 *            feature id from a feature. When not given, feature id is read
	 *            from feature.id.
	 * @return {boolean} True if the feature passes the conditions described by
	 *         the filter object.
	 */
  ...
  /**
	 * [scaleSelector description] The "standardized rendering pixel size" is
	 * defined to be 0.28mm × 0.28mm
	 * 
	 * @private
	 * @param {Rule}
	 *            rule
  ...
  /**
	 * getStyleNames, notice name is not required for userstyle, you might get
	 * undefined
	 * 
	 * @param {Layer}
	 *            layer [description]
  ...
  /**
	 * get style from array layer.styles, if name is undefined it returns
	 * default style. null is no style found
	 * 
	 * @param {Layer}
	 *            layer [description]
	 * @param {string}
  ...
  /**
	 ...
	 * 
	 * @example const style = getStyle(sldLayer, stylename);
	 ...
	 * @param {Function}
	 *            options.getProperty An optional function with parameters
	 *            (feature, propertyName) that can be used to extract a property
	 *            value from a feature. When not given, properties are read from
	 *            feature.properties directly.Error
	 * @param {Function}
	 *            options.getFeatureId An optional function to extract the
	 *            feature id from a feature.Error When not given, feature id is
	 *            read from feature.id.
	 * @return {Rule[]}
	 */
  ...
  /**
	 * Get all symbolizers inside a given rule. Note: this will be a mix of
	 * Point/Line/Polygon/Text symbolizers.
	 * 
	 * @param {object}
	 *            rule SLD rule object.
	 * @returns {Array<object>} Array of all symbolizers in a rule.
	 */
  function getRuleSymbolizers(rule) {
    // Helper for adding a symbolizer to a list when the symbolizer can be an
	// array of symbolizers.
  ...
  /**
	 * Gets a nested property from an object according to a property path. Note:
	 * path fragments may not contain a ".". Note: returns undefined if input
	 * obj is falsy.
	 * 
	 * @private
	 * @example getByPath({ a: { b: { c: 42 } } }, "a.b.c") // returns 42.
	 *          getByPath({ a: { b: { c: 42 } } }, "a.d.c") // returns
	 *          undefined, because obj.a has no property .d.
	 * @param {object}
	 *            obj Object.
	 * @param {string}
	 *            path Property path.
	 * @returns {any} Value of property at given path inside object, or
	 *          undefined if any property in the path does not exist on the
	 *          object.
	 */
  ...
  /**
	 ...
	 * @name GeometryStyles
	 * @description contains for each geometry type the symbolizer from an array
	 *              of rules
	 * @property {PolygonSymbolizer[]} polygon polygonsymbolizers
	 * @property {LineSymbolizer[]} line linesymbolizers
	 * @property {PointSymbolizer[]} point pointsymbolizers, same as graphic
	 *           prop from PointSymbolizer
	 */
  ...
  /**
	 * @private Global image cache. A map of image Url -> { url: image url,
	 *          image: an Image instance containing image data, width: image
	 *          width in pixels, height: image height in pixels }
	 */
  var imageCache = {};
  ...
  /**
	 * @private Global image loading state cache. A map of image Url -> one of
	 *          'IMAGE_LOADING', 'IMAGE_LOADED', 'IMAGE_ERROR'
	 */
  ...
  function invalidateExternalGraphicSymbolizers(symbolizer, imageUrl) {
    // Look at all possible paths where an externalgraphic may be present within
	// a symbolizer.
    // When such an externalgraphic has been found, and its url equals imageUrl,
	// invalidate the symbolizer.
    for (var k = 0; k < externalGraphicPaths.length; k += 1) {
      // Note: this process assumes that each symbolizer has at most one
  ...
  function updateSymbolizerInvalidatedState(ruleSymbolizer, imageUrl) {
    ...
    // Watch out! A symbolizer inside a rule may be a symbolizer, or an array of
	// symbolizers.
    // Todo: refactor so rule.symbolizers property is always an array with 0..n
	// symbolizer objects.
  ...
  /**
	 * @private Invalidate all symbolizers inside a featureTypeStyle's rules
	 *          having an ExternalGraphic matching the image url
	 * @param {object}
	 *            featureTypeStyle A feature type style object.
	 * @param {string}
  ...
  /**
	 * @private Load and cache an image that's used as externalGraphic inside
	 *          one or more symbolizers inside a feature type style object. When
	 *          the image is loaded, the symbolizers with ExternalGraphics
	 *          pointing to the image are invalidated, and the
	 *          imageLoadedCallback is called with the loaded image url.
	 * @param {url}
	 *            imageUrl Image url.
	 * @param {object}
	 *            featureTypeStyle Feature type style object.
	 * @param {Function}
	 *            imageLoadedCallback Will be called with the image url when
	 *            image has loaded. Will be called with undefined if the loading
	 *            the image resulted in an error.
	 */
  ...
  /**
	 * @private Start loading images used in rules that have a pointsymbolizer
	 *          with an externalgraphic.
	 * @param {Array
	 *            <object>} rules Array of SLD rule objects that pass the filter
	 *            for a single feature.
	 * @param {FeatureTypeStyle}
	 *            featureTypeStyle The feature type style object for a layer.
	 * @param {Function}
	 *            imageLoadedCallback Function to call when an image has loaded.
	 */
  function processExternalGraphicSymbolizers(
    ...
    // Dive into the symbolizers to find ExternalGraphic elements and for each
	// ExternalGraphic,
      ...
      allSymbolizers.forEach(function (symbolizer) {
        externalGraphicPaths.forEach(function (path) {
          ...
          if (!imageLoadingState) {
            // Start loading the image and set image load state on the
			// symbolizer.
            setImageLoadingState(imageUrl, IMAGE_LOADING);
  ...
  /**
	 * @private Create an OL Icon style for an external graphic. The Graphic
	 *          must be already loaded and present in the global imageCache.
	 * @param {string}
	 *            imageUrl Url of the external graphic.
	 * @param {number}
	 *            size Requested size in pixels.
	 * @param {number}
	 *            [rotationDegrees] Image rotation in degrees (clockwise).
	 *            Default 0.
	 */
  function createCachedImageStyle(imageUrl, size, rotationDegrees) {
    ...
    return new style.Style({
      image: new style.Icon({
        ...
        // According to SLD spec, if size is given, image height should equal
		// the given size.
        scale: size / height || 1,
  ...
  /**
	 * @private Function to memoize style conversion functions that convert sld
	 *          symbolizers to OpenLayers style instances. The memoized version
	 *          of the style converter returns the same OL style instance if the
	 *          symbolizer is the same object. Uses a WeakMap internally. Note:
	 *          This only works for constant symbolizers.
	 * @param {Function}
	 *            styleFunction Function that accepts a single symbolizer object
	 *            and returns the corresponding OpenLayers style object.
	 * @returns {Function} The memoized function of the style conversion
	 *          function.
	 */
  function memoizeStyleFunction(styleFunction) {
    ...
    return function (symbolizer) {
      ...
      // Create a new style if no style has been created yet, or when
		// symbolizer has been invalidated.
  ...
  /**
	 * @private Convert a hex color (like #AABBCC) to an rgba-string.
	 * @param {string}
	 *            hex eg #AA00FF
	 * @param {Number}
	 *            alpha eg 0.5
  ...
  /**
	 * @private Create an OL point style corresponding to a well known symbol
	 *          identifier.
	 * @param {string}
	 *            wellKnownName SLD Well Known Name for symbolizer. Can be
	 *            'circle', 'square', 'triangle', 'star', 'cross', 'x',
	 *            'hexagon', 'octagon'.
	 * @param {number}
	 *            size Symbol size in pixels.
	 * @param {ol/style/stroke}
	 *            stroke OpenLayers Stroke instance.
	 * @param {ol/style/fill}
	 *            fill OpenLayers Fill instance.
	 * @param {number}
	 *            rotationDegrees Symbol rotation in degrees (clockwise).
	 *            Default 0.
	 */
    ...
    switch (wellKnownName) {
      ...
      default:
        ...
        return new style.RegularShape({
          ...
          fill: fill,
          points: 4,
          // For square, scale radius so the height of the square equals the
			// given size.
  ...
  // This module contains an evaluate function that takes an SLD expression
	// and a feature and outputs the value for that feature.
  // Constant expressions are returned as-is.

  /**
	 * @private Evaluate the value of a sub-expression.
	 * @param {object}
	 *            childExpression SLD object expression child.
	 * @param {ol/feature}
	 *            feature OpenLayers feature instance.feature.
	 */
  ...
  /**
	 * @private This function takes an SLD expression and an OL feature and
	 *          outputs the expression value for that feature. Constant
	 *          expressions are returned as-is.
	 * @param {object|string}
	 *            expression SLD object expression.
	 * @param {ol/feature}
	 *            feature OpenLayers feature instance.
	 */
  function evaluate(expression, feature) {
    ...
    // In case of multiple child expressions, concatenate the evaluated child
	// results.
    var childValues = [];
  ...
  /**
	 * @private Utility function for evaluating dynamic expressions without a
	 *          feature. If the expression is static, the expression value will
	 *          be returned. If the expression is dynamic, defaultValue will be
	 *          returned. If the expression is falsy, defaultValue will be
	 *          returned.
	 * @param {object|string}
	 *            expression SLD object expression (or string).
	 * @param {any}
	 *            defaultValue Default value.
	 * @returns {any} The value of a static expression or default value if the
  ...
  /**
	 * Get an OL style/Stroke instance from the css/svg properties of the
	 * .stroke property of an SLD symbolizer object.
	 * 
	 * @private
	 * @param {object}
	 *            stroke SLD symbolizer.stroke object.
	 * @return {object} OpenLayers style/Stroke instance. Returns undefined when
	 *         input is null or undefined.
	 */
  function getSimpleStroke(stroke) {
    // According to SLD spec, if no Stroke element is present inside a
	// symbolizer element,
  ...
  /**
	 * Get an OL style/Fill instance from the css/svg properties of the .fill
	 * property of an SLD symbolizer object.
	 * 
	 * @private
	 * @param {object}
	 *            fill SLD symbolizer.fill object.
	 * @return {object} OpenLayers style/Fill instance. Returns undefined when
	 *         input is null or undefined.
	 */
  function getSimpleFill(fill) {
    // According to SLD spec, if no Fill element is present inside a symbolizer
	// element,
  ...
  /**
	 ...
	 *            pointsymbolizer [description]
	 * @return {object} openlayers style
	 */
  function pointStyle(pointsymbolizer) {
    var style$1 = pointsymbolizer.graphic;

    // If the point size is a dynamic expression, use the default point size and
	// update in-place later.
    var pointSizeValue = expressionOrDefault(style$1.size, DEFAULT_MARK_SIZE);

    // If the point rotation is a dynamic expression, use 0 as default rotation
	// and update in-place later.
    ...
    if (style$1.externalgraphic && style$1.externalgraphic.onlineresource) {
      ...
      // In that case, set pointSizeValue to null, so no scaling is calculated
		// for the image.
      if (!style$1.size) {
        pointSizeValue = null;
      }
      ...
      switch (getImageLoadingState(imageUrl)) {
        ...
        default:
          // A symbolizer should have loading state metadata, but return
			// IMAGE_LOADING just in case.
          return imageLoadingPointStyle;
      }
    }
  ...
  /**
	 * @private Get an OL point style instance for a feature according to a
	 *          symbolizer.
	 * @param {object}
	 *            symbolizer SLD symbolizer object.
	 * @param {ol/Feature}
	 *            feature OpenLayers Feature.
	 * @returns {ol/Style} OpenLayers style instance.
	 */
  function getPointStyle(symbolizer, feature) {
    // According to SLD spec, when a point symbolizer has no Graphic, nothing
	// will be rendered.
    ...
    if (size && size.type === 'expression') {
      ...
      if (graphic.externalgraphic && graphic.externalgraphic.onlineresource) {
        ...
        // Note: only ol/style/Circle has a setter for radius. RegularShape does
		// not.
        olImage.setRadius(sizeValue * 0.5);
      } else {
        // For a non-Circle RegularShape, create a new olImage in order to
		// update the size.
        olImage = getWellKnownSymbol(
          (graphic.mark && graphic.mark.wellknownname) || 'square',
          sizeValue,
          // Note: re-use stroke and fill instances for a (small?) performance
			// gain.
          olImage.getStroke(),
          olImage.getFill()
        );
  ...
  function splitLineString(geometry, minSegmentLength, options) {
    ...
    function calculateAngle(startNode, nextNode, alwaysUp) {
      ...
      if (!alwaysUp) {
        ...
        // angle = y > 0 ? angle + Math.PI : x < 0 ? angle + Math.PI * 2 :
		// angle;
      }
      return angle;
    }
  ...
  function patchRenderer(renderer) {
    ...
    // Add setImageStyle2 function that does the same as setImageStyle, except
	// that it sets rotation
    // to a given value instead of taking it from imageStyle.getRotation().
    // This fixes a problem with re-use of the (cached) image style instance
	// when drawing
    // many points inside a single line feature that are aligned according to
  ...
  /**
	 ...
	 * 
	 * @private
	 * @param {ol/render/canvas/Immediate}
	 *            render Instance of CanvasImmediateRenderer used to paint
	 *            stroke marks directly to the canvas.
	 * @param {Array
	 *            <Array<number>>} pixelCoords A line as array of [x,y] point
	 *            coordinate arrays in pixel space.
	 * @param {number}
	 *            minSegmentLength Minimum segment length in pixels for
	 *            distributing stroke marks along the line.
	 * @param {ol/style/Style}
	 *            pointStyle OpenLayers style instance used for rendering stroke
	 *            marks.
	 * @param {number}
	 *            pixelRatio Ratio of device pixels to css pixels.
	 * @returns {void}
	 */
  function renderStrokeMarks(render, pixelCoords, minSegmentLength, pointStyle, pixelRatio) {
    ...
    // The first element of the first pixelCoords entry should be a number
	// (x-coordinate of first point).
    // If it's an array instead, then we're dealing with a multiline or
	// (multi)polygon.
    // In that case, recursively call renderStrokeMarks for each child
	// coordinate array.
  ...
  /**
	 * Create a renderer function for renderining GraphicStroke marks to be used
	 * inside an OpenLayers Style.renderer function.
	 * 
	 * @private
	 * @param {LineSymbolizer}
	 *            linesymbolizer SLD line symbolizer object.
	 * @returns {ol/style/Style~RenderFunction} A style renderer function
	 *          (pixelCoords, renderState) => void.
	 */
  function getGraphicStrokeRenderer(linesymbolizer) {
    ...
    // Use strokeDasharray to space graphics. First digit represents size of
	// graphic, second the relative space, e.g.
    // size = 20, dash = [2 6] -> 2 ~ 20 then 6 ~ 60, total segment length
	// should be 20 + 60 = 80
    var multiplier = 1; // default, i.e. a segment is the size of the graphic
						// (without stroke/outline).
  ...
  /**
	 * Create an OpenLayers style for rendering line symbolizers with a
	 * GraphicStroke.
	 * 
	 * @private
	 * @param {LineSymbolizer}
  ...
  /**
	 ...
	 * @param {object}
	 *            symbolizer SLD symbolizer object.
	 * @return {object} OpenLayers style instance corresponding to the stroke of
	 *         the given symbolizer.
	 */
  ...
  /**
	 * @private Get an OL line style instance for a feature according to a
	 *          symbolizer.
	 * @param {object}
	 *            symbolizer SLD symbolizer object.
	 * @returns {ol/Style} OpenLayers style instance.
  ...
  function getExternalGraphicFill(symbolizer) {
    ...
    switch (getImageLoadingState(fillImageUrl)) {
      ...
      default:
        // Load state of an image should be known at this time, but return
		// 'loading' style as fallback.
        return imageLoadingPolygonStyle.getFill();
    }
  }
  ...
  /**
	 * @private Get the static OL style instance for a text symbolizer. The text
	 *          and placement properties will be set on the style object at
	 *          runtime.
	 * @param {object}
	 *            textsymbolizer SLD text symbolizer object.
	 * @return {object} openlayers style
	 */
  function textStyle(textsymbolizer) {
    ...
    var textStyleOptions = {
      text: stringDivider(labelText, cantidadCaracteresSeparacion, '\n'),
  ...
  // USAMOS ESTA FUNCION PARA PARTIR UN STRING (str) QUE OCUPE MAS
  // DE X CANTIDAD DE CARACTERES (width)
  // SE MANDA /n EN spaceReplacer
  ...
  /**
	 * @private Get an OL text style instance for a feature according to a
	 *          symbolizer.
	 * @param {object}
	 *            symbolizer SLD symbolizer object.
	 * @param {ol/Feature}
	 ...
	 * @returns {ol/Style} OpenLayers style instance.
	 */
  function getTextStyle(symbolizer, feature, layerId) {
	let context = document.querySelector("wc-map");
	let mostrarEtiquetas = getLayerByLayerId(context, layerId).getProperties()["mostrarEtiquetas"];
    ...
    if (label && label.type === 'expression') {
      ...
      let cantidadSeparadores = 0;
      let separadoresPrevios = '';
      if(labelText.length>cantidadCaracteresSeparacion){
    	  cantidadSeparadores = Math.ceil(labelText.length/cantidadCaracteresSeparacion);
      }
      labelText = stringDivider(labelText, cantidadCaracteresSeparacion, '\n');
      for (var k = 0; k < cantidadSeparadores; k ++) {
    	  separadoresPrevios = separadoresPrevios+'\n';
      }
      // Important! OpenLayers expects the text property to always be a
		// string.
      
      if(mostrarEtiquetas){
    	  olText.setText(separadoresPrevios+labelText.toString());    	  
      }
      else{
    	  olText.setText("");    	  
      }
    ...
    if (labelplacement) {
      ...
      if (pointPlacementRotation.type === 'expression') {
        ...
        olText.setRotation((Math.PI * labelRotationDegrees) / 180.0); // OL
																		// rotation
																		// is in
																		// radians.
      }
  ...
  /**
	 * @private Convert symbolizers together with the feature to OL style
	 *          objects and append them to the styles array.
	 * @example appendStyle(styles, point[j], feature, getPointStyle);
	 * @param {Array
	 *            <ol/style>} styles Array of OL styles.
	 * @param {object|Array
	 *            <object>} symbolizers Feature symbolizer object, or array of
	 *            feature symbolizers.
	 * @param {ol/feature}
	 *            feature OpenLayers feature.
	 * @param {Function}
	 *            styleFunction Function for getting the OL style object.
	 *            Signature (symbolizer, feature) => OL style.
	 */
  ...
  /**
	 ...
	 * @param {GeometryStyles}
	 *            GeometryStyles rulesconverter
	 * @param {object|Feature}
	 *            feature {@link http://geojson.org|geojson} or
	 *            {@link https://openlayers.org/en/latest/apidoc/module-ol_Feature-Feature.html|ol/Feature}
	 *            Changed in 0.0.04 & 0.0.5!
	 * @return ol.style.Style or array of it
	 */
  function OlStyler(GeometryStyles, feature, layerId) {
    ...
    switch (geometryType) {
      ...
      case 'MultiPoint':
        ...
        for (var j$1 = 0; j$1 < text.length; j$1 += 1) {
          styles.push(getTextStyle(text[j$1], feature, layerId));
        }
        break;
      ...
      case 'MultiLineString':
        ...
        for (var j$3 = 0; j$3 < text.length; j$3 += 1) {
          styles.push(getTextStyle(text[j$3], feature, layerId));
        }
        break;
      ...
      case 'MultiPolygon':
        ...
        for (var j$4 = 0; j$4 < text.length; j$4 += 1) {
          styles.push(getTextStyle(text[j$4], feature, layerId));
        }
        break;
  ...
  /**
	 * @private Extract feature id from an OpenLayers Feature.
	 * @param {Feature}
	 *            feature
	 *            {@link https://openlayers.org/en/latest/apidoc/module-ol_Feature-Feature.html|ol/Feature}
  ...
  /**
	 * @private Extract a property value from an OpenLayers Feature.
	 * @param {Feature}
	 *            feature
	 *            {@link https://openlayers.org/en/latest/apidoc/module-ol_Feature-Feature.html|ol/Feature}
	 * @param {string}
	 *            propertyName The name of the feature property to read.
  ...
  /**
	 * Create an OpenLayers style function from a FeatureTypeStyle object
	 * extracted from an SLD document.
	 * 
	 * **Important!** When using externalGraphics for point styling, make sure
	 * to call .changed() on the layer inside options.imageLoadedCallback to
	 * immediately see the loaded image. If you do not do this, the image icon
	 * will only become visible the next time OpenLayers draws the layer (after
	 * pan or zoom).
	 * 
	 * @param {FeatureTypeStyle}
	 *            featureTypeStyle Feature Type Style object.
	 ...
	 *            options Options
	 * @param {function}
	 *            options.convertResolution An optional function to convert the
	 *            resolution in map units/pixel to resolution in meters/pixel.
	 *            When not given, the map resolution is used as-is.
	 * @param {function}
	 *            options.imageLoadedCallback Optional callback that will be
	 *            called with the url of an externalGraphic when an image has
	 *            been loaded (successfully or not). Call .changed() inside the
	 *            callback on the layer to see the loaded image.
	 * @returns {Function} A function that can be set as style function on an
	 *          OpenLayers vector style layer.
	 * @example myOlVectorLayer.setStyle(SLDReader.createOlStyleFunction(featureTypeStyle, {
  ...
  function createOlStyleFunction(featureTypeStyle, options) {
    ...
    return function (feature, mapResolution) {
      ...
      // Determine applicable style rules for the feature, taking feature
		// properties and current resolution into account.
      var rules = getRules(featureTypeStyle, feature, resolution, {
        getProperty: getOlFeatureProperty,
        getFeatureId: getOlFeatureId,
      });

      // Start loading images for external graphic symbolizers and when
		// loaded:
      ...
      // Convert style rules to style rule lookup categorized by geometry
		// type.
      var geometryStyles = getGeometryStyles(rules);

      // Determine style rule array.
      var olStyles = OlStyler(geometryStyles, feature, options.layerId);
